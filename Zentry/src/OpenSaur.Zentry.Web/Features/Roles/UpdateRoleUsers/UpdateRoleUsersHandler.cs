...
public static class UpdateRoleUsersHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var lockAcquired = await lockService.TryAcquireLockAsync(LockKeys.RoleUsers(request.RoleId), LockTimeout, cancellationToken);
        if (!lockAcquired)
        {
            return AppHttpResults.Conflict("Role user assignments in progress.", "Another assignment operation is currently in progress for this role. Please try again.");
        }
        ...
        {
            await lockService.ReleaseLockAsync(LockKeys.RoleUsers(request.RoleId), cancellationToken);
        }
    }
}
