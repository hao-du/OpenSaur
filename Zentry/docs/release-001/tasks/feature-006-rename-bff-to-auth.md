...
- [x] Refactor folder `src/OpenSaur.Zentry.Web/Features/Bff` to `src/OpenSaur.Zentry.Web/Features/Auth`.
- [x] Rename routes from `/bff/login` and `/bff/logout` to `/auth/login` and `/auth/logout` (`MapAuthEndpoints`).
- [x] Update cookie name from `__Host-zentry-bff` to `zentry-s` in `Program.cs`.
- [x] Update authentication scheme constants, cache keys, and lock prefixes to avoid "bff" keyword.

### Item 2: Frontend & Vite Configuration Update
- [x] Update `client/src/features/auth/hooks/useAuth.ts` and `UserProfileMenu.tsx` to use `/auth/login` and `/auth/logout`.
