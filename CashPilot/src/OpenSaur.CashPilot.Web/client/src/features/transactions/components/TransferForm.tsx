...
function calculateTransferAmount(details: TransferDetailEditor[]) {
  const netAmount = details.reduce((sum, detail) => {
    const amount = Number(detail.amount);
    if (!Number.isFinite(amount)) {
      return sum;
    }

    return sum + (Number(detail.direction) === transactionDirectionValues.inflow ? amount : -amount);
  }, 0);
...
export function TransferForm({
  ...
  const initialValuesSignature = JSON.stringify({
    counterparties: counterparties.map((x) => x.id),
  ...
  const calculatedAmount = calculateTransferAmount(details);
  ...
  }, [counterparties, currencies, movementInitialDetails, movementInitialTransactionItems, movementInitialValue]);
  ...
  const handleSave = async () => {
    ...
    if (details.length === 0) {
      headerForm.setError("amount", {
        type: "validate",
        message: t("transactions.validation.transferTransactionRequired"),
      });
      return;
    }
    ...
    if (calculatedAmount <= 0) {
      headerForm.setError("amount", {
        type: "validate",
        message: t("transactions.validation.transferAmountPositive"),
      });
      return;
    }
    if (
