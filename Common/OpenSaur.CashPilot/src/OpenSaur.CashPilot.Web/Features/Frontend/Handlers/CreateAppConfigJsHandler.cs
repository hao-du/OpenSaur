...
using OpenSaur.CashPilot.Web.Features.Frontend.Dtos;
...
)
{
    ...
    public Task<IResult> HandleAppConfigJs()
    {
        ...
        return Task.FromResult<IResult>(TypedResults.Text(
            $"window.__CASHPILOT_CONFIG__ = Object.freeze({JsonSerializer.Serialize(runtimeConfig, RuntimeConfigSerializerOptions)});",
